...
plt.xlabel("Episodes")
plt.ylabel("Steps taken by agent")

def run(env,color,label,episodes,a,b,rew=1, is_training=True):
    ...
    for t in range(episodes):
        sum_rewards[t] = np.sum(steps_per_episode[max(0, t-100):(t+1)])
    # plt.clf()
    plt.plot(sum_rewards,color=color,label=label)
    plt.legend(loc='center right', bbox_to_anchor=(1.25, 0.5))
    plt.tight_layout()
    # plt.savefig("./Individual_Graphs/"+label+'.png')
    plt.savefig("Steps_per_100.png")
...
def run_env(a,b,color):
    ...
    run(env=env,color=color,label="("+str(a)+","+str(b)+")",a=a,b=b,episodes=50000)
